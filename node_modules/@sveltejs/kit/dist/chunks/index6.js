...
const compile$1 = (ast, options = {}) => {
  let walk = (node, parent = {}) => {
    let invalidBlock = utils$7.isInvalidBrace(parent);
    let invalidNode = node.invalid === true && options.escapeInvalid === true;
    ...
    if (node.nodes && node.ranges > 0) {
      let args = utils$7.reduce(node.nodes);
...
const expand$1 = (ast, options = {}) => {
  ...
  let walk = (node, parent = {}) => {
    ...
    let enclose = utils$6.encloseBrace(node);
...
async function make_package(config, cwd = process.cwd()) {
	...
	const pkg = JSON.parse(fs.readFileSync(path$2.join(cwd, 'package.json'), 'utf8'));
	...
	for (const file of files) {
		...
		const filename = path$2.join(config.kit.files.lib, file);
		const source = fs.readFileSync(filename, 'utf8');
...
function load_tsconfig(filename, ts) {
	const filedir = path$2.dirname(filename);
	...
	const { options } = ts.parseJsonConfigFileContent(
		config,
		ts.sys,
		path$2.dirname(tsconfig_filename),
		{ sourceMap: false },
		tsconfig_filename
	);
	return options;
